          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="p-2.5 rounded-lg bg-cyan-500/10 border border-cyan-500/30">
                ...
                <div className="text-lg font-bold text-cyan-50">{reportedQuota}</div>
              </div>
              <div className="p-2.5 rounded-lg bg-green-500/10 border border-green-500/30">
                ...
                <div className="text-lg font-bold text-green-400">{totalProjects}</div>
              </div>
              <div className="p-2.5 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
                ...
                <div className="text-lg font-bold text-yellow-400">{remainingQuota}</div>
              </div>
              <div className="p-2.5 rounded-lg bg-purple-500/10 border border-purple-500/30">
                ...
                <div className="text-lg font-bold text-purple-400">{annualReported}</div>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="grid grid-cols-4 gap-2">
                {quarterlyConversionRate.map((item, index) => (
                  <div
                    key={index}
                    className="p-2 rounded-lg bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-cyan-500/30"
                  >
                    <div className="text-xl font-bold text-cyan-400">{item.rate}%</div>
          ...
          {showOverdueDetails ? (
                ...
                <tbody>
                  {overdueProjects.map((project, index) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-2 text-cyan-50 font-medium">{project.name}</td>
                      <td className="py-2 px-2 text-center text-cyan-300">{project.stage}</td>
                      <td className="py-2 px-2 text-center">
                        <span className="px-2 py-1 rounded-md bg-red-500/20 text-red-400 font-semibold">
                          {project.overdue}天
                        </span>
                      </td>
                      <td className="py-2 px-2 text-right text-cyan-50 font-semibold">{project.amount}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              {overdueProjects.map((project) => (
                <div
                  key={project.id}
                  className="p-3 rounded-lg bg-red-500/10 border border-red-500/30 hover:bg-red-500/20 transition-colors"
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-red-300">{project.id}</span>
                    <span className="text-xs text-red-400 font-semibold">{project.overdue}天</span>
                  </div>
                  <div className="text-sm font-medium text-red-50 truncate mb-1">{project.name}</div>
                  <div className="text-xs text-red-300/70">{project.stage}</div>
                </div>
              ))}
            </div>
          )}
